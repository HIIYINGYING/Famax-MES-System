...
async function loadItems() {
  loading.value = true;
  const { data } = await supabase.from("oqc_inspections").select("*, customers(customer_name)").order("inspection_date", { ascending: false });
  items.value = data ?? [];
  loading.value = false;
}
...
async function submitInspection() {
  ...
  const { error: err } = await supabase.from("oqc_inspections").update({
