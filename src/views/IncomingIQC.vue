<template>
  <div class="page">
    ...
    <div class="card">
      ...
      <table v-else>
        <thead><tr><th>DO Number</th><th>Item</th><th>Supplier</th><th>Received Qty</th><th>Receiving Date</th><th>Status</th><th></th></tr></thead>
        <tbody>
          ...
          <tr v-if="items.length === 0"><td colspan="7" class="empty">No items pending inspection.</td></tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="selected" class="modal-backdrop" @click.self="selected = null">
      <div class="modal">
        <h2>IQC Inspection — {{ selected.item }}</h2>
        <p class="sub">DO: {{ selected.do_no }} · Order: {{ selected.order_no || "-" }} · Received Qty: {{ selected.quantity }}</p>
...
async function loadItems() {
  loading.value = true;
  const { data } = await supabase.from("iqc_inspections").select("*, partners(name)").in("status", ["PENDING", "PENDING_INSPECTION", "REINSPECT"]).order("inspection_date", { ascending: false });
  items.value = data ?? [];
  loading.value = false;
}
...
async function submitInspection() {
  ...
  const { data: updated, error: err } = await supabase.from("iqc_inspections").update({
...
function validInspectionQuantities() {
  ...
  if (!Number.isInteger(accepted) || !Number.isInteger(rejected) || accepted < 0 || rejected < 0 ||
    ...
    formError.value = "Accepted and rejected quantities must be whole numbers that add up to the received quantity.";
    return false;
  }
  return true;
}
