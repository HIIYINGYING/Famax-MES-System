<template>
  <div class="page">
    ...
    <div class="card">
      ...
      <table v-else>
        <thead><tr><th>DO Number</th><th>Item</th><th>Order No</th><th>Customer</th><th>Qty</th><th>Status</th><th></th></tr></thead>
        <tbody>
          <tr v-for="i in items" :key="i.id">
            <td>{{ i.do_no || "-" }}</td>
            <td>{{ i.item }}</td>
            <td>{{ i.order_no || "-" }}</td>
            <td>{{ i.customers?.customer_name || "-" }}</td>
            <td>{{ i.quantity }}</td>
            <td><span class="pill" :class="statusClass(i.status)">{{ i.status }}</span></td>
            <td><a href="#" @click.prevent="openForm(i)">Inspect</a></td>
          </tr>
          <tr v-if="items.length === 0"><td colspan="7" class="empty">No orders pending final inspection.</td></tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="selected" class="modal-backdrop" @click.self="selected = null">
      <div class="modal">
        <h2>OQC Inspection — {{ selected.item }}</h2>
        <p class="sub">Order: {{ selected.order_no }} · Qty: {{ selected.quantity }}</p>
        ...
        <select v-model="form.status">
          <option value="PASS_CHECK">Pass — Ready for Delivery</option>
...
async function loadItems() {
  loading.value = true;
  const { data } = await supabase.from("oqc_inspections").select("*, customers(customer_name)").in("status", ["PENDING", "PENDING_INSPECTION", "REINSPECT"]).order("inspection_date", { ascending: false });
  items.value = data ?? [];
  loading.value = false;
}
...
async function submitInspection() {
  ...
  const { data: updated, error: err } = await supabase.from("oqc_inspections").update({
...
function validInspectionQuantities() {
  ...
  if (!Number.isInteger(accepted) || !Number.isInteger(rejected) || accepted < 0 || rejected < 0 ||
    ...
    formError.value = "Accepted and rejected quantities must be whole numbers that add up to the delivery quantity.";
    return false;
  }
  return true;
}
